...
def test_socket_connect_missing_address(get_by_name, register_socket, engage):
    ...
    when_called = manager.connect.when.called_with('foobar', '', 'some-mechanism')

    # Then it should have raised an exception
    when_called.should.have.raised(SocketConnectError, 'socket "foobar" received an empty address and cannot connect')
...
def test_socket_ensure_and_connect(get_or_create, connect, engage):
    ...
    connect.assert_called_once_with('foobar', 'inproc://whatevs', 'some-mechanism')
...
def test_socket_bind_missing_address(get_by_name, register_socket, engage):
    ...
    # Then it should have raised an exception
    when_called.should.have.raised(SocketBindError, 'socket "foobar" received an empty address and cannot bind')
...
def test_socket_bind_error(get_by_name, register_socket, engage):
    ...
    # When I call bind
    when_called = manager.bind.when.called_with('foobar', 'bad-address', 'some-mechanism')
...
def test_socket_connect_error(get_by_name, register_socket, engage):
    ...
    # When I call connect
    when_called = manager.connect.when.called_with('foobar', 'bad-address', 'some-mechanism')
